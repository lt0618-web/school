<template>
	...
	<view class="order-dom" v-for="(item,index) in productInfo" :key="index" :index="index">
		<view class="order">
			...
			<view class="cont">
				...
				<view class="cont_sel">
					<view class="item">
						...
						<view class="right">{{item.mode}}</view>
					</view>
					<view class="item">
			...
			<view class="foot">
				<text>共{{item.count}}件 小计：￥{{totalPrice(item.count,item.money,item.freight)}}</text>
			</view>
		</view>
	</view>
	{{orderCountAll(productInfo)}}{{orderMoneyAll(productInfo)}}
	</view>
</template>
